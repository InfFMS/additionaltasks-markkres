...
def run_test(input_file: str, expected_output_file: str):
    ...
    result = subprocess.run(
        ['python', 'task5.py'],  # Команда для запуска
    ...
    for i in range(len(expected_output) - 1):
        if expected_output[i] > '9':
            expected_output_2 = expected_output[i + 1:] + expected_output[:i + 1]
            break

    for c in program_output:
        if (c < '0' or (c > '9' and c not in 'NSWE')):
            print(colored(f"Test {input_file} failed.", "red"))
            print(colored(f"Incorrect symbol in output", "yellow"))
            return False

    # Проверка правильности ответа
    if program_output == expected_output or program_output == expected_output_2:
